function h = clickA3DPointDS(V,F)
%CLICKA3DPOINT
...
figure;
h = gcf;
% trisurf(F,V(:,1), V(:,2), V(:,3),'Facecolor','none','Edgecolor',[0.7,0.7,0.7]);
trisurf(F,V(:,1), V(:,2), V(:,3));
axis equal
% cameratoolbar('Show'); % show the camera toolbar

hold on; % so we can highlight clicked points without clearing the figure
% disp(lower(get(h,'selectiontype')))
% fcwDS(h,V,F,{'','pan','','zoom'})
fcwDS(h,V,F,{'','pan','rot','zoom'})
return
